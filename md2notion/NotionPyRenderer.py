...
from notion.block import (BulletedListBlock, CodeBlock, CollectionViewBlock,
                          DividerBlock, HeaderBlock, ImageBlock,
                          NumberedListBlock, QuoteBlock, SubheaderBlock,
                          SubsubheaderBlock, TextBlock)
...
class NotionPyRenderer(BaseRenderer):
    ...
    def renderMultipleToString(self, tokens):
        ...
        def toString(renderedBlock):
            if isinstance(renderedBlock,
                          dict) and renderedBlock['type'] == TextBlock:
                return renderedBlock[
                    'title']  #This unwraps TextBlocks/paragraphs to use in other blocks
    ...
    def render_block_code(self, token):
        ...
        notionSoLangs = [
            ...
            "JSON", "Kotlin", "LaTeX", "Less", "Lisp", "LiveScript", "Lua",
            "Makefile", "Markdown", "Markup", "MATLAB", "Nix", "Objective-C",
            "OCaml", "Pascal", "Perl", "PHP", "Plain Text", "PowerShell",
            "Prolog", "Python", "R", "Reason", "Ruby", "Rust", "Sass", "Scala",
            "Scheme", "Scss", "Shell", "SQL", "Swift", "TypeScript", "VB.Net",
            "Verilog", "VHDL", "Visual Basic", "WebAssembly", "XML", "YAML"
        ]
        if token.language != "":
            matchLang = next(
                (lang for lang in notionSoLangs
        ...
        def blockFunc(blockStr):
            return {
                'type': CodeBlock,
                'language': matchLang,
                'title_plaintext': blockStr.rstrip()
            }
    ...
    def render_heading(self, token):
        ...
        def blockFunc(blockStr):
            return {
                'type': [HeaderBlock, SubheaderBlock,
                         SubsubheaderBlock][level - 1],
                'title': blockStr
            }
    ...
    def render_quote(self, token):
        def blockFunc(blockStr):
            return {'type': QuoteBlock, 'title': blockStr}
    ...
    def render_list_item(self, token):
        leaderContainsNumber = re.match(r'\d',
                                        token.leader)  #Contains a number
        ...
        return {
            'type':
            NumberedListBlock if leaderContainsNumber else BulletedListBlock,
            'title': "".join(strings),
            'children': children
        }
    ...
    def render_table(self, token):
        ...
        # },
        schema = {
            randColId(): textColSchema(headerRow[r])
            for r in range(len(headerRow) - 1)
        }
    ...
    def render_strong(self, token):
        return self.renderMultipleToStringAndCombine(token.children,
                                                     lambda s: f"**{s}**")
